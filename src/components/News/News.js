...
export default function News() {
    ...
    const CustomPagination = () => (
        <ReactPaginate
            ...
            containerClassName={
                "pagination react-paginate pagination-sm justify-content-center pr-1 mt-4"
            }
        />
    );
            ...
            <Row className="m-0 p-0">
                <Col md="12" sm="12">
                    {newsList?.map((item, index) => (
                        <Card key={item.id} className="card-posts" style={{ cursor: "pointer" }} onClick={() => { specificNews(item.id) }}>
                            <CardBody>
                                <h5>{item.title}</h5>
                                <p>{item.body}</p>
